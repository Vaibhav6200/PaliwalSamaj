...
from SamajApp.tasks import translate_member_fields
...
def generate_address(state_name, city_name, village_name):
    ...
    try:
        ...
        state, _ = State.objects.get_or_create(state_name=state_name, state_name_hi=state_name, state_name_en=state_name_en)

        # Handle city (if entered, it must be linked to a state)
        if city_name:
            city_name_en = transliterate.hindi2english(city_name)
            city, _ = City.objects.get_or_create(city_name=city_name, city_name_hi=city_name, city_name_en=city_name_en, state=state)

        # Handle village (if entered, it must be linked to a city)
        if village_name:
            village_name_en = transliterate.hindi2english(village_name)
            village, _ = Village.objects.get_or_create(village_name=village_name, village_name_hi=village_name, village_name_en=village_name_en, city=city)
        ...
        with ((((transaction.atomic())))):
            for i, (_, row) in enumerate(df.iterrows()):
                ...
                if full_name:
                    ...
                    member.full_name_en = transliterate.hindi2english(full_name).replace("palival", "paliwal")
                    member.full_name_hi = full_name
                if birth_place:
                    member.birth_place=birth_place
                    member.birth_place_en=transliterate.hindi2english(birth_place)
                    member.birth_place_hi=birth_place
                if father_name:
                    member.father_name = father_name
                    member.father_name_en = transliterate.hindi2english(father_name)
                    member.father_name_hi = father_name
                if current_address:
                    member.current_address = current_address
                    member.current_address_en = transliterate.hindi2english(current_address)
                    member.current_address_hi = current_address
                member.save()
                ...
                if occupation_type == 'job':
                    occupation_obj = OccupationDetail(member=member)
                    if occupation:
                        occupation_obj.job_description=occupation
                        occupation_obj.job_description_hi=occupation
                        occupation_obj.job_description_en=transliterate.hindi2english(occupation)
                    occupation_obj.save()
                    print("💼 Job details added")
                elif occupation_type == 'business':
                    occupation_obj = OccupationDetail(member=member)
                    if occupation:
                        occupation_obj.business_description=occupation
                        occupation_obj.business_description_hi=occupation
                        occupation_obj.business_description_en=transliterate.hindi2english(occupation)
                    occupation_obj.save()
